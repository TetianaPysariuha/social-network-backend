...
public interface   FriendJpaRepository extends RepositoryInterface<Friend>, JpaSpecificationExecutor<Friend> {
    ...
    @Query(value ="with userfriends as (select CASE WHEN friend_id = :id THEN user_id" +
           " END friendID,status " +
                  "  from friends ff" +
            " where :id in (ff.user_id, ff.friend_id))" +
            " select * from users u" +
            ...
            "and :id not in (fr.user_id, fr.friend_id)" +
    ...
    HashSet<User> FriendsForFriendship(@Param("id")Long userId);


    @Query(value = "select u.* from users u join users uauth on u.city = uauth.city and u.country = uauth.country " +
            "where uauth.id = :userId limit 50")
    HashSet<User> searchFriendsForCity(Long userId);
