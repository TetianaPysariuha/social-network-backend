    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .authorizeHttpRequests(
                        authz -> authz
                                .requestMatchers("/api/auth/login","/api/auth/**", "/api/auth/token","/api/auth","/api/auth/passwordLetter","/api/auth/refresh",
                                                 "/swagger-ui/**","api/oauth2/authorization/google","/posts", "/websocket-endpoint/**", "/topic/messages").permitAll()
